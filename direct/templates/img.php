					...
					<table width="100%">
						<tr>
							...
							<td width="85" style="text-align:center;">
								<?php if($im["image"] != "") { ?>
									<a href="/images/files/<?=$im['image']?>" rel="lightbox" title="<?=$im['title']?>"><!--rel="lightbox[img]"-->
...
else if(isset($_GET["upload"]))
{
	...
	<script language="JavaScript">
		...
		//alert (<?=count($_FILES['goods_img1']['name'])?>);
		</script>
	<?
	if(count($_FILES['goods_img1']['name']) > 0) 
	{
		$imindex = 0;
		//foreach($_FILES as $key)
		for($key=0;$key<sizeof($_FILES['goods_img1']['name']);$key++)
		{
			...
			$imindex++;
			$dot = strrpos($_FILES['goods_img1']['name'][$key],'.');
			$title = substr($_FILES['goods_img1']['name'][$key],0,$dot);
			$_FILES['goods_img1']['name'][$key] = $sourceid."_".$parent."_".date("dmYHis")."_$imindex.jpg";
			$uploadfile = $uploaddir.basename($_FILES['goods_img1']['name'][$key]);
			move_uploaded_file($_FILES['goods_img1']["tmp_name"][$key],$uploadfile);
			...
			if(file_exists($uploadfile))
			{
				...
				db_query("INSERT INTO images (parentid,image,is_main,source,showorder,width,height,format,title) VALUES ('".$parent."','".($_FILES['goods_img1']['name'][$key])."','$is_main','$sourceid','".($max + 1)."','$width','$height','$format','$title')");	
			}
			//$key++;
		}
		?>
...
<table>
	<tr>
		...
		<td><input style="width:200px;" type="file" id="goods_img1" multiple name="goods_img1[]" />&nbsp;<select onchange="show_file(this)">
		<?php
		for($i = 1;$i < 21;$i++)
		{
			?>
